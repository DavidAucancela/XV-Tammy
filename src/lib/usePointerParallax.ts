 ...
 *
 * `enabled` (default true) lets a caller opt out entirely — the motion
 * values then stay at rest and no listener is attached, so the background
 * they drive no longer reacts to the cursor.
 */
export function usePointerParallax(enabled = true): { x: MotionValue<number>; y: MotionValue<number> } {
